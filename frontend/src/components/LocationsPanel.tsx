...
import { faChevronLeft, faChevronRight, faCircleXmark, faEye, faSave, faTrash } from "@fortawesome/free-solid-svg-icons";
...
export default function LocationsPanel() {
  ...
  const handleUpdateLocation = async (locationId: number) => {
    ...
    if (changes.item_name !== undefined) {
      payload.item_name = typeof changes.item_name === "string" && changes.item_name.trim() === "" ? null : changes.item_name;
    }
              ...
              : currentLocations.map((location) => (
                  <tr key={location.id}>
                    ...
                    <td>
                      <input
                        type="text"
                        value={String(getCurrentValue(location.id, "item_name", location.item_name ?? "") ?? "")}
                        onChange={(e) => updateLocationField(location.id, "item_name", e.target.value)}
                        className="editable-input"
                      />
                    </td>
                    <td>
                      <select
                        value={String(getCurrentValue(location.id, "spot_type", location.spot_type))}
                        onChange={(e) => updateLocationField(location.id, "spot_type", e.target.value as SpotType)}
                    ...
                    <td>
                      <input
                        type="text"
                        value={String(getCurrentValue(location.id, "latitude", location.latitude ?? "") ?? "")}
                        onChange={(e) => updateLocationField(location.id, "latitude", e.target.value)}
                        className="editable-input"
                        placeholder="-"
                      />
                    </td>
                    <td>
                      <input
                        type="text"
                        value={String(getCurrentValue(location.id, "longitude", location.longitude ?? "") ?? "")}
                        onChange={(e) => updateLocationField(location.id, "longitude", e.target.value)}
                        className="editable-input"
                        placeholder="-"
                      />
                    </td>
                    <td className="action-buttons">
                      <button className="view-btn" onClick={() => navigate(`/location/${location.id}`)}>
                        <FontAwesomeIcon icon={faEye} />
                      </button>
                      <button className="update-btn" onClick={() => handleUpdateLocation(location.id)}>
                        <FontAwesomeIcon icon={faSave} />
                      </button>
                      <button className="delete-btn" onClick={() => setDeleteModalOpen(location.id)}>
