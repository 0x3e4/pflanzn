...
const normalizeSpotType = (value: string) => value.trim().toLowerCase().replace(/[\s_-]+/g, "");
const toHashtag = (value: string) => `#${value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "")}`;
...
export default function LocationDetails() {
  ...
  const derivedTags = useMemo(() => {
    ...
    const tags = [
      toHashtag(spotTypeLabels[location.spot_type]),
      toHashtag(location.visibility),
    ];
  ...
  const handleCoordinateBlur = async (
  ...
  const handleIdentifyFromLatestImage = async () => {
    ...
    setIdentifying(true);
    try {
      const imageResponse = await fetch(`/api/uploads/${latestImage.image_path}`);
      if (!imageResponse.ok) {
        throw new Error("Failed to load location image for identification.");
      }

      const imageBlob = await imageResponse.blob();
      const identifyFile = new File([imageBlob], `location-${location.id}.jpg`, { type: imageBlob.type || "image/jpeg" });
  ...
  const addCustomTag = () => {
    ...
    const exists = [...customTags, ...derivedTags.map((tag) => tag.replace(/^#/, ""))]
      .some((tag) => tag.toLowerCase() === next.toLowerCase());
    if (exists) {
      setNewTag("");
  ...
  const handleTagInputChange = (value: string) => {
    ...
    const nextFiltered = allKnownTagNames
      .filter((tag) => tag.includes(normalized) && !used.has(tag))
      .slice(0, 6);
  ...
  const handleTagSuggestionClick = (tagName: string) => {
    ...
    persistCustomTags((prev) => {
      const exists = [...prev, ...derivedTags.map((tag) => tag.replace(/^#/, ""))]
        .some((tag) => tag.toLowerCase() === tagName.toLowerCase());
      if (exists) {
        return prev;
      }
      return [...prev, tagName];
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      try {
        ...
        const next = parsed.filter(
          (entry) => String(entry || "").trim().toLowerCase().replace(/^#+/, "") !== normalized,
        );
            ...
            <h2>
              {isLoggedIn ? (
                <span
                  ...
                  onBlur={(event) => handleLocationNameBlur(event.currentTarget.innerText, event.currentTarget)}
                  tabIndex={0}
                >
                  {location.name}
                </span>
            ...
            <span className="location-information-names plant-information-names">
              ...
              {isLoggedIn ? (
                <span
                  ...
                  contentEditable
                  suppressContentEditableWarning
                  onBlur={(event) => handleSpotTypeBlur(event.currentTarget.innerText, event.currentTarget)}
                  tabIndex={0}
                >
            ...
            <span className="location-information-names plant-information-names">
              ...
              {isLoggedIn ? (
                <span
                  ...
                  contentEditable
                  suppressContentEditableWarning
                  onBlur={(event) => handleItemNameBlur(event.currentTarget.innerText, event.currentTarget)}
                  tabIndex={0}
            ...
            <span className="location-information-names plant-information-names">
              ...
              {isLoggedIn ? (
                <span
                  ...
                  contentEditable
                  suppressContentEditableWarning
                  onBlur={(event) => handleCoordinateBlur("latitude", event.currentTarget.innerText, event.currentTarget)}
                  tabIndex={0}
                ...
                <span className="editable-input-noauth">{formatCoordinate(location.latitude) || "Not set"}</span>
              )}
            </span>
            ...
            <span className="location-information-names plant-information-names">
              ...
              {isLoggedIn ? (
                <span
                  ...
                  contentEditable
                  suppressContentEditableWarning
                  onBlur={(event) => handleCoordinateBlur("longitude", event.currentTarget.innerText, event.currentTarget)}
                  tabIndex={0}
                ...
                <span className="editable-input-noauth">{formatCoordinate(location.longitude) || "Not set"}</span>
              )}
            </span>
          </div>

          <LocationTimelineImages locationId={location.id} images={location.images} onChanged={loadLocation} />
          <LocationCalendar
                ...
                <div className="location-tag-dropdown">
                  {filteredTags.map((tag) => {
                    const isTagUsed = allTags.some(
                      (existingTag) => existingTag.replace(/^#/, "").toLowerCase() === tag.toLowerCase(),
                    );

                    return (
                      <div key={tag} className={`location-tag-dropdown-item${isTagUsed ? " disabled" : ""}`}>
                        ...
                        {isLoggedIn && (
                          <FontAwesomeIcon
                            ...
                            onMouseDown={(event) => {
                              ...
                              if (isTagUsed) {
                                toast.warning(`"${tag}" is used on this location and cannot be deleted.`);
                                setNewTag("");
                                return;
                              }
                              handleDeleteKnownTag(tag);
                            }}
                            title={isTagUsed ? "Tag is in use on this location" : "Delete tag"}
                          />
                        )}
                      </div>
                    );
                ...
                activityEvents.map((activity) => (
                  <div key={activity.id} className="location-activity-item">
                    <div className={`location-activity-icon ${activity.type}`}>
                      <FontAwesomeIcon
                        icon={activity.type === "image" ? faCamera : activity.type === "note" ? faStickyNote : faSeedling}
                        size="sm"
                      />
                    </div>
            ...
            {isLoggedIn ? (
              <>
                ...
                <button
                  ...
                  onClick={() => document.getElementById(`file-upload-location-${location.id}`)?.click()}
                >
                  <FontAwesomeIcon icon={faUpload} /> Upload Image
                </button>

                <button className="identify-location-btn-bottom" onClick={handleIdentifyFromLatestImage} disabled={identifying}>
                  <FontAwesomeIcon icon={faFingerprint} /> {identifying ? "Identifying..." : "Identify Crop/Herb"}
                </button>
                ...
                {import.meta.env.VITE_LLM_PROVIDER && (
                  ...
                  >
                    <FontAwesomeIcon icon={faWandMagicSparkles} /> {generatingDescription ? "Generating..." : "AI Description"}
                  </button>
                )}
              </>
            ) : (
              <>
                <div className="season-event-input-container">
                  <button className="season-event-btn" onClick={() => toast.warning("You must be logged in to add season events.")}>
                    <FontAwesomeIcon icon={faSeedling} /> Add Harvest Time
                  </button>
                  <div className="season-event-datetime">
                    <input
                ...
                <button className="file-input-location-btn" onClick={() => toast.warning("You must be logged in to upload images.")}>
                  <FontAwesomeIcon icon={faUpload} /> Upload Image
                </button>
                <button className="identify-location-btn-bottom" onClick={() => toast.warning("You must be logged in to identify locations.")}>
                ...
                {import.meta.env.VITE_LLM_PROVIDER && (
                  <button
                    className="generate-location-description-btn"
                    onClick={() => toast.warning("You must be logged in to generate an AI description.")}
                  >
